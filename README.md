...
服务层提供了一种组织查询逻辑的标准方式，通过继承 `AbstractService` 类可以快速创建针对特定表的查询服务。

### 基本用法

详细的服务层使用示例和最佳实践，请参考[测试文档](test/README.MD)，其中包含了完整的示例代码和设计说明。

### 配置参数

服务类支持传入配置参数，可以在创建查询构建器时使用：
...
// 设置配置参数
$service->setConfig([
    'pool_name' => 'clickhouse_analytics',
    'max_running_processes' => 10
]);

// 使用配置参数创建查询
$data = $service->getData(['status' => 1]);
```

### 最佳实践

1. **表名常量化**：使用表类中定义的常量作为表名
2. **方法语义化**：方法名应清晰表达其功能和查询意图
3. **参数验证**：在服务方法中添加必要的参数验证
4. **异常处理**：使用 try-catch 包裹查询操作
